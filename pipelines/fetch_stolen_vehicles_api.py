...
def load_intelligence_datalake():
    ...
    if not os.path.exists(AST_LOGS_PATH):
        logging.warning("ast_logs.json not found! Cannot cross-reference geographic locations.")
        return {}
    
    try:
        with open(AST_LOGS_PATH, 'r', encoding='utf-8') as f:
            data = json.load(f)
            # Create a speedy lookup dictionary indexed by the strict Case ID (e.g. "AK26013048")
            lookup_table = {}
